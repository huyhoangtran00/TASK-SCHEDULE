...
export const useBoardStore = create((set) => ({
  ...
  fetchBoards: async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:45000/api/boards', {
  ...
  try {
    const token = localStorage.getItem('token');
    const response = await fetch(`http://localhost:45000/api/boards/${id}`, {
      headers: { Authorization: token },
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.message);
  ...
  updateBoard: async (boardId, boardData) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:45000/api/boards/${boardId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: token,
